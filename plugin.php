...
EventManager::getInstance()->listen('router.register', function ($router) {
    ...
    $router->get('/password/forgot', [UserController::class, 'forgot'], [GuestMiddleware::class]);
    $router->get('/password/change', [UserController::class, 'reset'], [GuestMiddleware::class]);
    ...
    $router->post('/password/change', [UserController::class, 'change'], [GuestMiddleware::class]);
});
